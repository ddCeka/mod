    ...
    if len(expandObjectVars):
        ...
        hbox:
            ...
            hbox:
                ...
                textbutton "{mod_notl}Search{/mod_notl}" style_suffix "buttonPrimary" yalign .5 action Function(mod.Search.doSearch)
                textbutton "{mod_notl}Reset{/mod_notl}" yalign .5 action Function(mod.Search.resetSearch)
        ...
        if len(mod.Search.results) > 0:
            ...
            use mod_tableRow():
                hbox xsize colWidth[0]:
                    hbox:
                        ...
                        if nameSorted == 'asc':
                            textbutton '{size=-6}\ue316{/size}' yoffset mod.scalePxInt(-4) style_suffix 'icon_textbutton' hovered mod.Tooltip('{mod_notl}Sort descending{/mod_notl}') unhovered mod.Tooltip() action [Function(mod.Search.sort, reverse=True),SetLocalVariable('nameSorted', 'desc')]
                        else:
                            textbutton If(nameSorted,'{size=-6}\ue313{/size}','{size=-6}\ue5d7{/size}') yoffset mod.scalePxInt(-4) style_suffix 'icon_textbutton' hovered mod.Tooltip('{mod_notl}Sort ascending{/mod_notl}') unhovered mod.Tooltip() action [Function(mod.Search.sort),SetLocalVariable('nameSorted', 'asc')]
                hbox xsize colWidth[1]:
                    label If(mod.Search.searchType=='labels', '{mod_notl}Replay{/mod_notl}', '{mod_notl}Value{/mod_notl}')
                        ...
                        use mod_tableRow(i, True):
                            if mod.Search.searchType == 'labels': # Are we searching labels?
                                ...
                                hbox spacing 2 yalign .5:
                                    use mod_iconButton('\ue1c4', '{mod_notl}Play{/mod_notl}', Show('mod_replay', labelName=var.name))
                                    use mod_iconButton('\ue163', '{mod_notl}Jump{/mod_notl}', Show('mod_jump', labelName=var.name))
                                    # Remember
                                    if mod.LabelsStore.has(var.name):
                                        use mod_iconButton('\ue4f8', '{mod_notl}Forget{/mod_notl}', Function(mod.LabelsStore.forget, var.name))
                                    else:
                                        use mod_iconButton('\ue862', '{mod_notl}Remember{/mod_notl}', Show('mod_remember_var', varName=var.name, rememberType='label'))
            ...
            vbox:
                ...
                label "{mod_notl}No results{/mod_notl}" xalign 0.5
...
screen mod_search_options():
    ...
    use mod_Dialog('Search options', closeAction=Hide('mod_search_options'), modal=True, icon='\ue8b6'):
        text '{mod_notl}Search type:{/mod_notl}'
        vbox spacing 2:
            use mod_radiobutton(checked=(mod.Search.searchType=='variable names'), text='{mod_notl}Variable names{/mod_notl}', action=[SetField(mod.Search, 'searchType', 'variable names'),Hide('mod_search_options')])
            use mod_radiobutton(checked=(mod.Search.searchType=='values'), text='{mod_notl}Values{/mod_notl}', action=[SetField(mod.Search, 'searchType', 'values'),Hide('mod_search_options')])
            use mod_radiobutton(checked=(mod.Search.searchType=='labels'), text='{mod_notl}Labels/Scenes{/mod_notl}', action=[SetField(mod.Search, 'searchType', 'labels'),Hide('mod_search_options')])
        ...
        vbox spacing 2:
            hbox:
                ...
                textbutton "\ueb8b" style_suffix "icon_button" yalign .5 hovered mod.Tooltip("Explain resursive search") unhovered mod.Tooltip() action mod.Confirm("""Enabling this feature means you'll search previous results until you reset""", title='Recursive search')
            hbox:
